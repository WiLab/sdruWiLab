  ...
  errMsg_size[2])
{
  ...
  int32_T i;
  const mxArray *y;
  static const int32_T iv154[2] = { 1, 6 };

  const mxArray *m24;
  char_T cv175[6];
  static const char_T cv176[6] = { 's', 'i', 'l', 'e', 'n', 't' };
  ...
  if (!isSetupsdruCalled) {
    y = NULL;
    m24 = mxCreateCharArray(2, iv154);
    for (i = 0; i < 6; i++) {
      cv175[i] = cv176[i];
    }

    emlrtInitCharArrayR2013a(&b_st, 6, m24, cv175);
    emlrtAssign(&y, m24);
...
void b_SDRuReceiver_stepImpl(testMACTransmitterStackData *SD, const emlrtStack
  *sp, const comm_SDRuReceiver *obj, creal_T y[5120])
{
  real_T fc;
  real_T loOffset;
  real_T gain;
  ...
  st.site = &pl_emlrtRSI;
  receiveData(&st, obj->pDriverHandle, fc, loOffset, gain, decim,
              SD->u1.f6.yTemp, &dataLen, &overrun, &errStatus, errMsg_data,
              errMsg_size);
  if (errStatus != UsrpDriverSuccess) {
    st.site = &pl_emlrtRSI;
    b_error(&st, errMsg_data, errMsg_size);
  ...
  for (i = 0; i < 5120; i++) {
    y[i].re = SD->u1.f6.yTemp[i].re;
    y[i].im = SD->u1.f6.yTemp[i].im;
  }
