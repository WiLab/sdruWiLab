...
{
  return mclFeval(_mcr_inst, "CreateObj", nlhs, plhs, nrhs, prhs);
}

LIB_libPassPointer_CPP_API 
void MW_CALL_CONV WriteDataToPipe(int nargout, mwArray& result, const mwArray& pointer)
{
  mclcppMlfFeval(_mcr_inst, "WriteDataToPipe", nargout, 1, 1, &result, &pointer);
}

LIB_libPassPointer_CPP_API 
void MW_CALL_CONV CreateObj(int nargout, mwArray& obj)
{
  mclcppMlfFeval(_mcr_inst, "CreateObj", nargout, 1, 0, &obj);
}
