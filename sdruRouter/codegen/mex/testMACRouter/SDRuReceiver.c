  ...
  comm_SDRuReceiver *obj)
{
  ...
  for (k = 0; k < 10; k++) {
    ...
    emlrtDynamicBoundsCheckFastR2012b(i14, 0, 255, &h_emlrtBCI, &b_st);
  }
  ...
  errMsg_size[2])
{
  ...
  int32_T i;
  const mxArray *y;
  static const int32_T iv109[2] = { 1, 6 };
  ...
  if (!isSetupsdruCalled) {
    ...
    emlrtInitCharArrayR2013a(&b_st, 6, m17, cv118);
    emlrtAssign(&y, m17);
...
void SDRuReceiver_stepImpl(testMACRouterStackData *SD, const emlrtStack *sp,
...
void b_SDRuReceiver_stepImpl(testMACRouterStackData *SD, const emlrtStack *sp,
  const comm_SDRuReceiver *obj)
{
  ...
  st.site = &ml_emlrtRSI;
  receiveData(&st, obj->pDriverHandle, fc, loOffset, gain, decim,
              SD->u1.f4.yTemp, &dataLen, &overrun, &errStatus, errMsg_data,
              errMsg_size);
  if (errStatus != UsrpDriverSuccess) {
    st.site = &ml_emlrtRSI;
    error(&st, errMsg_data, errMsg_size);
