...
static emlrtRSInfo nl_emlrtRSI = { 1, "SDRuReceiver",
  "/Users/travis/Documents/sdru/+comm/SDRuReceiver.p" };

static emlrtRSInfo tl_emlrtRSI = { 6, "openDataConnection",
  ...
  comm_SDRuReceiver *obj)
{
  ...
  int32_T i15;
  static const char_T cv110[5] = { 'S', 'D', 'R', 'u', '_' };
  ...
  b_st.site = &tj_emlrtRSI;
  for (k = 0; k < 10; k++) {
    i15 = (int32_T)varargin_1[k];
    emlrtDynamicBoundsCheckFastR2012b(i15, 0, 255, &emlrtBCI, &b_st);
  }

  b_st.site = &tj_emlrtRSI;
  for (k = 0; k < 5; k++) {
    c_obj->ObjectID[k] = cv110[k];
  }
  ...
  errMsg_size[2])
{
  ...
  int32_T i;
  const mxArray *y;
  static const int32_T iv100[2] = { 1, 6 };
  ...
  if (!isSetupsdruCalled) {
    y = NULL;
    m15 = mxCreateCharArray(2, iv100);
    for (i = 0; i < 6; i++) {
      cv111[i] = cv112[i];
    }

    emlrtInitCharArrayR2013a(&b_st, 6, m15, cv111);
    emlrtAssign(&y, m15);
...
void SDRuReceiver_stepImpl(const emlrtStack *sp, const comm_SDRuReceiver *obj,
  creal_T y[5120])
{
  real_T fc;
  real_T loOffset;
  ...
  st.site = &nl_emlrtRSI;
  receiveData(&st, obj->pDriverHandle, fc, loOffset, gain, decim, yTemp,
