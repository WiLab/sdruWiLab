...
static emlrtRSInfo kf_emlrtRSI = { 7, "OFDMletters2bits",
...
static emlrtBCInfo j_emlrtBCI = { -1, -1, 11, 27, "letter", "OFDMletters2bits",
  "/Volumes/git/traviscollins/sdruWiLab/sdruTxRx/private/OFDMletters2bits.m", 0
};

static emlrtRSInfo yv_emlrtRSI = { 11, "OFDMletters2bits",
  "/Volumes/git/traviscollins/sdruWiLab/sdruTxRx/private/OFDMletters2bits.m" };
  ...
  emlrtMCInfo *location)
{
  ...
  const mxArray *m42;
  pArray = b;
  return emlrtCallMATLABR2012b(sp, 1, &m42, 1, &pArray, "str2double", TRUE,
    location);
}
...
void OFDMletters2bits(const emlrtStack *sp, const char_T str[80], real_T f[560])
{
  ...
  for (firstcol = 0; firstcol < 80; firstcol++) {
    for (i = 0; i < 7; i++) {
      i0 = 1 + i;
      emlrtDynamicBoundsCheckFastR2012b(i0, 1, bits_size_idx_1, &j_emlrtBCI, sp);
      y = NULL;
...
void b_OFDMletters2bits(const emlrtStack *sp, real_T f[560])
{
  ...
  int32_T i13;
  int32_T firstcol;
  static const char_T cv68[80] = { 'L', 'i', 'v', 'e', ' ', 'l', 'o', 'n', 'g',
  ...
  for (firstcol = 0; firstcol < 80; firstcol++) {
    for (i = 0; i < 7; i++) {
      i13 = 1 + i;
      emlrtDynamicBoundsCheckFastR2012b(i13, 1, bits_size_idx_1, &j_emlrtBCI, sp);
      y = NULL;
      m11 = emlrtCreateString1(bits_data[firstcol + 80 * i]);
      emlrtAssign(&y, m11);
      st.site = &yv_emlrtRSI;
      f[firstcol + 80 * i] = emlrt_marshallIn(&st, str2double(&st, y,
        &b_emlrtMCI), "str2double");
