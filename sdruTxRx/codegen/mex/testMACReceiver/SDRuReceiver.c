...
static emlrtRSInfo re_emlrtRSI = { 1, "SDRuReceiver",
...
comm_SDRuReceiver *SDRuReceiver_SDRuReceiver(comm_SDRuReceiver *obj)
{
  ...
  char_T temp[10];
  int32_T i14;
  static const char_T cv99[5] = { 'S', 'D', 'R', 'u', '_' };
  ...
  y = NULL;
  m18 = mxCreateDoubleScalar(1.0);
  emlrtAssign(&y, m18);
  b_y = NULL;
  m18 = mxCreateDoubleScalar(10.0);
  emlrtAssign(&b_y, m18);
  ...
  for (k = 0; k < 10; k++) {
    i14 = (int32_T)r[k];
    i14 = emlrtDynamicBoundsCheckFastR2012b(i14, 0, 255, &h_emlrtBCI,
      emlrtRootTLSGlobal);
    i14 = (int32_T)muDoubleScalarRem(i14, 256.0);
    if (i14 < 0) {
      temp[k] = (int8_T)-(int8_T)(uint8_T)-(real_T)i14;
    } else {
      temp[k] = (int8_T)(uint8_T)(real_T)i14;
    }
  }
  ...
  *deviceStatus, char_T errMsg_data[1024], int32_T errMsg_size[2])
{
  ...
  if (!isSetupsdruCalled) {
    ...
    emlrtInitCharArrayR2013a(emlrtRootTLSGlobal, 6, m20, cv121);
    emlrtAssign(&y, m20);
...
void SDRuReceiver_stepImpl(testMACReceiverStackData *SD, const comm_SDRuReceiver
  *obj, creal_T y[5120])
{
  real_T fc;
  real_T loOffset;
  ...
  emlrtPushRtStackR2012b(&re_emlrtRSI, emlrtRootTLSGlobal);
  receiveData(obj->pDriverHandle, fc, loOffset, gain, decim, SD->u1.f8.yTemp,
  ...
  for (i = 0; i < 5120; i++) {
    y[i].re = SD->u1.f8.yTemp[i].re;
    y[i].im = SD->u1.f8.yTemp[i].im;
    fc = y[i].im;
...
void b_SDRuReceiver_stepImpl(testMACReceiverStackData *SD, const
