...
static emlrtRSInfo kd_emlrtRSI = { 1, "SDRuTransmitter",
...
comm_SDRuTransmitter *SDRuTransmitter_SDRuTransmitter(comm_SDRuTransmitter *obj)
{
  ...
  for (k = 0; k < 10; k++) {
    i6 = (int32_T)r[k];
    i6 = emlrtDynamicBoundsCheckFastR2012b(i6, 0, 255, &h_emlrtBCI,
      emlrtRootTLSGlobal);
  ...
  UsrpErrorCapiEnumT *errStatus, char_T errMsg_data[1024], int32_T errMsg_size[2])
{
  ...
  const mxArray *m37;
  char_T cv218[6];
  static const char_T cv219[6] = { 's', 'i', 'l', 'e', 'n', 't' };
  ...
  if (!isSetupsdruCalled) {
    y = NULL;
    m37 = mxCreateCharArray(2, iv182);
    for (i = 0; i < 6; i++) {
      cv218[i] = cv219[i];
    }

    emlrtInitCharArrayR2013a(emlrtRootTLSGlobal, 6, m37, cv218);
    emlrtAssign(&y, m37);
...
void SDRuTransmitter_stepImpl(testMACReceiverStackData *SD, const
