...
static emlrtBCInfo k_emlrtBCI = { -1, -1, 11, 27, "letter", "OFDMletters2bits",
  "/Volumes/git/traviscollins/sdruWiLab/sdruTxRx/private/OFDMletters2bits.m", 0
};

static emlrtRSInfo dw_emlrtRSI = { 11, "OFDMletters2bits",
  "/Volumes/git/traviscollins/sdruWiLab/sdruTxRx/private/OFDMletters2bits.m" };
  ...
  emlrtMCInfo *location)
{
  ...
  const mxArray *m45;
  pArray = b;
  return emlrtCallMATLABR2012b(sp, 1, &m45, 1, &pArray, "str2double", TRUE,
    location);
}
...
void OFDMletters2bits(const emlrtStack *sp, const char_T str[80], real_T f[560])
{
  ...
  for (firstcol = 0; firstcol < 80; firstcol++) {
    for (i = 0; i < 7; i++) {
      i0 = 1 + i;
      emlrtDynamicBoundsCheckFastR2012b(i0, 1, bits_size_idx_1, &k_emlrtBCI, sp);
      y = NULL;
...
void b_OFDMletters2bits(const emlrtStack *sp, real_T f[560])
{
  ...
  for (firstcol = 0; firstcol < 80; firstcol++) {
    for (i = 0; i < 7; i++) {
      i13 = 1 + i;
      emlrtDynamicBoundsCheckFastR2012b(i13, 1, bits_size_idx_1, &k_emlrtBCI, sp);
      y = NULL;
