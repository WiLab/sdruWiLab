...
comm_SDRuTransmitter *SDRuTransmitter_SDRuTransmitter(comm_SDRuTransmitter *obj)
{
  ...
  for (k = 0; k < 10; k++) {
    i5 = (int32_T)r[k];
    i5 = emlrtDynamicBoundsCheckFastR2012b(i5, 0, 255, &emlrtBCI,
      emlrtRootTLSGlobal);
    i5 = (int32_T)muDoubleScalarRem(i5, 256.0);
    if (i5 < 0) {
      temp[k] = (int8_T)-(int8_T)(uint8_T)-(real_T)i5;
    } else {
      temp[k] = (int8_T)(uint8_T)(real_T)i5;
    }
  }
  ...
  UsrpErrorCapiEnumT *errStatus, char_T errMsg_data[1024], int32_T errMsg_size[2])
{
  ...
  if (!isSetupsdruCalled) {
    y = NULL;
    m22 = mxCreateCharArray(2, iv115);
    for (i = 0; i < 6; i++) {
      cv122[i] = cv123[i];
    }

    emlrtInitCharArrayR2013a(emlrtRootTLSGlobal, 6, m22, cv122);
    emlrtAssign(&y, m22);
